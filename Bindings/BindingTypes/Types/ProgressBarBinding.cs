...
public class ProgressBarBinding : IBindingTypeHandler
{
    ...
    {
        if (viewObject is not ProgressBar control)
        {
            return;
        }
    ...
    private void SetValue(object modelObject, string modelPropertyName, ProgressBar control)
    {
        object value = modelObject.GetType().GetProperty(modelPropertyName).GetValue(modelObject);

        control.Value = Convert.ToDouble(value);;
    }
